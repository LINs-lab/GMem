...
class LightningDiT(nn.Module):
    ...
    def __init__(
        ...
        self.use_gmem = use_gmem
        if use_gmem:
            # print(f"Using Gmem, dropout rate is {0.4}")
            self.condition_projection = FeatureEmbedder(input_size=768, hidden_size=hidden_size, dropout_prob=0.4)
        else:
        ...
        self.projector = nn.ModuleList([
            build_mlp(hidden_size, 2048, 768)
        ])
    ...
    def forward(self, x, t=None, y=None, c=None):
        """
        ...
        """
        assert (self.use_gmem and c is not None) or (not self.use_feature_condition), "Feature condition must be input if self.use_feature_condition"
        assert y is None, "Label embedding is not used in LightningDiT, please use c instead."
        ...
        if self.use_gmem:
            y = self.condition_projection(c, self.training)
        else:
        ...
        for i, block in enumerate(self.blocks):
            ...
            if (i + 1) == self.encoder_depth and self.training:
                z = [projector(x.reshape(-1, D)).reshape(N, T, -1) for projector in self.projector][0]
    ...
    def forward_with_cfg(self, x, t, y, c, cfg_scale, cfg_interval=None, cfg_interval_start=None, cfg_interval_end=None):
        """
        ...
        assert y is None, "In GMem, y should be None."
        model_out = self.forward(x=combined, t=t, y=y, c=c)
        ...
        if cfg_interval is True:
            ...
            if timestep < cfg_interval_start:
                half_eps = cond_eps
            elif cfg_interval_end is not None and timestep > cfg_interval_end:
                half_eps = uncond_eps
...
def LightningDiT_S_1(**kwargs): # only used for debugging
    print("Warning: LightningDiT_S_1 is only used for fast debugging.")
...
LightningDiT_models = {
    'LightningDiT-S/1': LightningDiT_S_1,
