...
class FeatureEmbedder(nn.Module):
    ...
    def forward(self, features: torch.Tensor, train: bool=False) -> torch.Tensor:
        """
...
class SiT(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs # fused_attn
        ...
        self.use_feature_condition = use_feature_condition
        ...
        if self.use_feature_condition:
            self.condition_projection = FeatureEmbedder(input_size=768, hidden_size=hidden_size, dropout_prob=dropout_rate)
        else:
    ...
    def initialize_weights(self):
        ...
        if self.use_feature_condition is False:
    ...
    def forward(self, x, t, y, feature_condition: Tensor=None, return_logvar=False, train=False):
        """
        Forward pass of SiT.
        ...
        assert (self.use_feature_condition and feature_condition is not None) or (not self.use_feature_condition), "Feature condition must be input if self.use_feature_condition"
        ...
        if self.use_feature_condition:
            y = self.condition_projection(feature_condition, train)
        else:
