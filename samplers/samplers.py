...
def euler_sampler(
        ...
        use_feature_condition=False,
        ):
    # setup conditioning
    if cfg_scale > 1.0:
        if use_feature_condition:
            assert False, "cfg is turned off when use_feature_condition"
    ...
    with torch.no_grad():
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            ...
            if cfg_scale > 1.0 and t_cur <= guidance_high and t_cur >= guidance_low:
                ...
                y_cur = y            
                if use_feature_condition:
                    feature_condition_cur = feature_condition
            kwargs = dict(y=y_cur)
            if use_feature_condition:
                kwargs["feature_condition"] = feature_condition_cur
                assert model_input.size(0) == feature_condition_cur.size(0), "time_input and feature_condition_cur size samplers.py line 85"
